<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["unid"])){
        ...
        clearstatcache();
        $unsize=filesize("news.txt");
        if (!empty($un_id)){
            $unflag=0;
            if($unsize>0){
                $unfile=fopen("news.txt", "r");
                $unarr=explode(", ",fread($unfile,$unsize));
                fclose($unfile);
                $unfile = fopen("news.txt", "w");
                $start=0;
                foreach($unarr as $x){
...
<div class="header">
    <img src="images/Logo.png" style="width:247.217px;height:150px;">
</div>
<div class="body">
    ...
    <div class="main">
            ...
            <section id="newsletter">
                <link rel="stylesheet" href="newslet.css">
